...
func repl() {
	...
	setup := func() {
		if game == nil || position == nil {
			game = donna.NewGameEx()
			position = game.CacheSize(64).Start(donna.White)
			fmt.Printf("%s\n", position)
		}
	}

	think := func() {
		if move := game.Think(10, position); move != 0 {
...
func benchmark() {
	...
	game := donna.NewGameEx(`Kd1,Qh2,Nh6,a4,g3,h7`, `Kg7,Qe4,Bf6,b7,e6,g6`)
	fmt.Printf("%s\n", game)
	...
	game = donna.NewGameEx(`Kg1,Rb7,Na4,a2,b2,f4,g2,g3`, `Kh7,Rd2,Bb6,a7,c5,c4,g7,h6`)
	fmt.Printf("%s\n", game)
	game.CacheSize(64).Think(12, game.Start(donna.Black))

	// Georg Rotlewi vs. Akiba Rubinstein, Lodz 1907, after 21 moves.
	// http://www.chessgames.com/perl/chessgame?gid=1119679
	game = donna.NewGameEx(`Kh1,Qe2,Ra1,Rf1,Bb2,Be4,Nc3,a3,b4,e5,f4,g2,h2`, `Kg8,Qe7,Rc8,Rd8,Bb6,Bb7,Ng4,a6,b5,e6,f7,g7,h7`)
	fmt.Printf("%s\n", game)
	game.CacheSize(64).Think(12, game.Start(donna.Black))
	...
	game = donna.NewGameEx(`Ke1,Qa3,Rd1,Rh1,Bc4,Bc5,Nf3,a2,d4,f2,g2,h2`, `Kg8,Qb6,Ra8,Rf8,Bg4,Bg7,Nc3,a7,b7,c6,f7,g6,h7`)
	fmt.Printf("%s\n", game)
	game.CacheSize(64).Think(12, game.Start(donna.Black))
	...
	game = donna.NewGameEx(`Kg1,Qc2,Ra1,Re1,Bc1,Bg2,Ng5,a2,b2,c3,d4,f2,g3,h2`, `Kg8,Qd6,Ra8,Rf8,Bc8,Nd5,Ng6,a7,b6,c4,e6,f7,g7,h7`)
	fmt.Printf("%s\n", game)
	game.CacheSize(64).Think(10, game.Start(donna.White))

	// Mikhail Botvinnik vs. Jose Raul Capablanca, AVRO 1936, after 29 moves.
	// Botvinnik played 30. Bb2-a3!
	game = donna.NewGameEx(`Kg1,Qe5,Bb2,Ng3,c3,d4,e6,g2,h2`, `Kg7,Qe7,Nb3,Nf6,a7,b6,c4,d5,g6,h7`)
	fmt.Printf("%s\n", game)
	game.CacheSize(64).Think(12, game.Start(donna.White))

}

func perft(depth int) (total int64) {
	p := donna.NewGameEx().Start(donna.White)
	start := time.Now()
	total = p.Perft(depth)
