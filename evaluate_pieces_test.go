...
func TestEvaluatePieces000(t *testing.T) {
	NewGame().Setup(`Ke4,Ne1,Ra1,Rh1,a2,b2,c2,f2,g2,h2`, `Ke8,e7`).Start(White).EvaluateWithTrace()
	baseline := eval.metrics[`-Rooks`].(Total).white

	// H1 rook boxed (can castle): 1x penalty.
	NewGame().Setup(`Ke1,Ra1,Rh1,a2,b2,c2,f2,g2,h2`, `Ke8,e7`).Start(White).EvaluateWithTrace()
	rooks := eval.metrics[`-Rooks`].(Total).white
	expect(t, rooks.minus(baseline), rookBoxed.times(-1))

	// H1 rook boxed (can't castle): 2x penalty.
	NewGame().Setup(`Ke1,Ra1,Rf1,a2,b2,c2,f2,g2,h2`, `Ke8,e7`).Start(White).EvaluateWithTrace()
