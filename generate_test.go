...
func TestGenerate010(t *testing.T) {
	p := NewGame(`Ka1,a2,b3,c4,d2,e6,f5,g4,h3`, `Kc1`).start()
	gen := NewMoveGen(p).generateMoves(p).rank(Move(0))
...
func TestGenerate110(t *testing.T) {
	p := NewGame(`Kd4,e4,Nf4,Bc4,Ra5,Qh5`, `Kd8,Qd5`).start()
	gen := NewMoveGen(p).generateCaptures(p).rank(Move(0))

	expect.Eq(t, gen.allMoves(), `[e4xd5 Nf4xd5 Bc4xd5 Ra5xd5 Qh5xd5 Kd4xd5]`)
}

func TestGenerate120(t *testing.T) {
	p := NewGame(`Kd4,e4,Nf4,Bc4,Ra5,Qh5`, `Kd8,Qd5,Rf5`).start()
	gen := NewMoveGen(p).generateCaptures(p).rank(Move(0))
...
func TestGenerate130(t *testing.T) {
	p := NewGame(`Kd4,e4,Nf4,Bc4,Ra5,Qh5`, `Kd8,Qd5,Rf5,Bg6`).start()
	gen := NewMoveGen(p).generateCaptures(p).rank(Move(0))
...
func TestGenerate140(t *testing.T) {
	p := NewGame(`Kd4,e4,Nf4,Bc4,Ra5,Qh5`, `Kd8,Qd5,Rf5,Bg6,Nh3`).start()
	gen := NewMoveGen(p).generateCaptures(p).rank(Move(0))
...
func TestGenerate150(t *testing.T) {
	p := NewGame(`Kd4,e4,Nf4,Bc4,Ra5,Qh5`, `Kd8,Qd5,Rf5,Bg6,Nh3,e2`).start()
	gen := NewMoveGen(p).generateCaptures(p).rank(Move(0))
...
func TestGenerate200(t *testing.T) {
	...
	black := NewMoveGen(p).generateMoves(p).validOnly()
