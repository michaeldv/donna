...
func TestGenerateMoves000(t *testing.T) {
	p := NewGame().start()
	gen := NewMoveGen(p).generateMoves(p)
...
func TestGenerateMoves020(t *testing.T) {
	p := NewGame(`a2,b3,c4,d2,e6,f5,g4,h3`, `a3,b4,c5,e7,f6,g5,h4,Kg8`).start()
	gen := NewMoveGen(p).generateMoves(p)
...
func TestGenerateMoves030(t *testing.T) {
	p := NewGame(`a2,e4,g2`, `b3,f5,f3,h3,Kg8`).start()
	gen := NewMoveGen(p).generateMoves(p)
...
func TestGenerateMoves031(t *testing.T) {
	...
	white := NewMoveGen(p).generateMoves(p)
	expect.Contain(t, white.allMoves(), `0-0`)

	p.color = Black
	black := NewMoveGen(p).generateMoves(p)
...
func TestGenerateMoves200(t *testing.T) {
	...
	white := NewMoveGen(p).pawnMoves(p)
	expect.Eq(t, white.allMoves(), `[a6-a7 b7-b8Q b7-b8N b7-b8R b7-b8B]`)

	p.color = Black
	black := NewMoveGen(p).pawnMoves(p)
...
func TestGenerateMoves220(t *testing.T) {
	...
	white := NewMoveGen(p).pawnCaptures(p)
	expect.Eq(t, white.allMoves(), `[g5xh6 a6xb7 f6xg7]`)

	p.color = Black
	black := NewMoveGen(p).pawnCaptures(p)
...
func TestGenerateMoves300(t *testing.T) {
	log(); defer log()
	p := NewGame().start()
	gen := NewMoveGen(p).generateMoves(p).validOnly()
