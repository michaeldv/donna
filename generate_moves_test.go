...
func TestGenerateMoves000(t *testing.T) {
        ...
        gen := NewGen(game.Start(White), 0).GenerateMoves()
...
func TestGenerateMoves031(t *testing.T) {
        game := NewGame().Setup(`Ke1,Rh1,h2`, `Ke8,Ra8,a7`)
        gen := NewGen(game.Start(White), 0).GenerateMoves()
        contains(t, gen.allMoves(), `0-0`)

        gen = NewGen(game.Start(Black), 0).GenerateMoves()
...
func TestGenerateMoves200(t *testing.T) {
        ...
        white := NewGen(game.Start(White), 1).pawnMoves(White)
        expect(t, white.allMoves(), `[a6-a7 b7-b8Q b7-b8R b7-b8B b7-b8N]`)

        black := NewGen(game.Start(Black), 1).pawnMoves(Black)
...
func TestGenerateMoves220(t *testing.T) {
        ...
        white := NewGen(game.Start(White), 1).pawnCaptures(White)
        expect(t, white.allMoves(), `[g5xh6 a6xb7 f6xg7]`)

        black := NewGen(game.Start(Black), 1).pawnCaptures(Black)
