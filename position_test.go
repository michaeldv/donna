...
func TestPosition000(t *testing.T) {
	...
	expect.Eq(t, p.id, uint64(0x1A734D7E0DD57DD8))
	expect.Eq(t, p.pid, uint64(0xC6918785471EC42C))
	expect.Eq(t, p.color, White)
	expect.Eq(t, p.enpassant, Square(0))
	expect.Eq(t, p.king[White], Square(E1))
	expect.Eq(t, p.king[Black], Square(E8))
...
func TestPosition010(t *testing.T) {
	...
	expect.Eq(t, p.color, White)
	expect.Eq(t, p.castles, bit(H1)|bit(H8))
	expect.Eq(t, p.enpassant, Square(0))
	expect.Eq(t, p.king[White], Square(E1))
	expect.Eq(t, p.king[Black], Square(E8))
...
func TestPosition020(t *testing.T) {
	...
	expect.Eq(t, p.enpassant, Square(E6))
	expect.Eq(t, p.king[White], Square(B2))
	expect.Eq(t, p.king[Black], Square(F8))
	expect.Eq(t, p.outposts[Pawn], bit(B3)|bit(C2)|bit(D5)|bit(F3))
	expect.Eq(t, p.outposts[Knight], bit(E2))
	expect.Eq(t, p.outposts[Bishop], maskNone)
	...
	expect.Eq(t, p.outposts[BlackKnight], maskNone)
	expect.Eq(t, p.outposts[BlackBishop], maskNone)
