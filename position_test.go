...
func TestPosition000(t *testing.T) {
	...
	expect.Eq(t, p.pid, uint64(0x37FC40DA841E1692))
	expect.Eq(t, p.color, White)
	expect.Eq(t, p.enpassant, 0)
	expect.Eq(t, p.white.home, E1)
	expect.Eq(t, p.black.home, E8)
	expect.Eq(t, p.castles, uint(0x0F))
	expect.Eq(t, p.white.pawns, bit(A2)|bit(B2)|bit(C2)|bit(D2)|bit(E2)|bit(F2)|bit(G2)|bit(H2))
	expect.Eq(t, p.white.knights, bit(B1)|bit(G1))
	expect.Eq(t, p.white.bishops, bit(C1)|bit(F1))
	expect.Eq(t, p.white.rooks, bit(A1)|bit(H1))
	expect.Eq(t, p.white.queens, bit(D1))
	expect.Eq(t, p.white.king, bit(E1))
	expect.Eq(t, p.black.pawns, bit(A7)|bit(B7)|bit(C7)|bit(D7)|bit(E7)|bit(F7)|bit(G7)|bit(H7))
	expect.Eq(t, p.black.knights, bit(B8)|bit(G8))
	expect.Eq(t, p.black.bishops, bit(C8)|bit(F8))
	expect.Eq(t, p.black.rooks, bit(A8)|bit(H8))
	expect.Eq(t, p.black.queens, bit(D8))
	expect.Eq(t, p.black.king, bit(E8))
}
...
func TestPosition010(t *testing.T) {
	...
	expect.Eq(t, p.white.home, E1)
	expect.Eq(t, p.black.home, E8)
	expect.Eq(t, p.white.pawns, bit(A2)|bit(B4)|bit(F2)|bit(G2)|bit(H2))
	expect.Eq(t, p.white.knights, bit(D5)|bit(G1))
	expect.Eq(t, p.white.bishops, bit(G5))
	expect.Eq(t, p.white.rooks, bit(D1)|bit(H1))
	expect.Eq(t, p.white.queens, bit(E4))
	expect.Eq(t, p.white.king, bit(E1))
	expect.Eq(t, p.black.pawns, bit(A7)|bit(B7)|bit(F7)|bit(G7)|bit(H7))
	expect.Eq(t, p.black.knights, bit(C6))
	expect.Eq(t, p.black.bishops, bit(E6)|bit(F8))
	expect.Eq(t, p.black.rooks, bit(C8)|bit(H8))
	expect.Eq(t, p.black.queens, bit(B5))
	expect.Eq(t, p.black.king, bit(E8))
}
...
func TestPosition020(t *testing.T) {
	...
	expect.Eq(t, p.white.home, B2)
	expect.Eq(t, p.black.home, F8)
	expect.Eq(t, p.white.pawns, bit(B3)|bit(C2)|bit(D5)|bit(F3))
	expect.Eq(t, p.white.knights, bit(E2))
	expect.Eq(t, p.white.bishops, maskNone)
	expect.Eq(t, p.white.rooks, bit(D2))
	expect.Eq(t, p.white.queens, bit(G6))
	expect.Eq(t, p.white.king, bit(B2))
	expect.Eq(t, p.black.pawns, bit(A7)|bit(D6)|bit(E5)|bit(H5))
	expect.Eq(t, p.black.knights, maskNone)
	expect.Eq(t, p.black.bishops, maskNone)
	expect.Eq(t, p.black.rooks, bit(B8)|bit(C8))
	expect.Eq(t, p.black.queens, bit(C7))
	expect.Eq(t, p.black.king, bit(F8))
}
